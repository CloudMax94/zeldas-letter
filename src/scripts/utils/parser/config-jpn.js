...
export const controlCodes = {
    ...
    toBinary: (str) => {
      if (str.charAt() === '[') {
        ...
        if (str.slice(0, close.length) === close) {
          return [close.length, [0x00, 0x0B, 0x0C, 0x00]]
        }
        ...
        if (match) {
          ...
          return [match[0].length, [0x00, 0x0B, (color >> 8) & 0xFF, color & 0xFF]]
        }
      }
    },
    toMarkup: (argument) => {
  ...
  0x869F: {
    ...
    render: (state, argument) => {
      switch (argument) {
        case 0: return state.write('１０００')
        case 1: return state.write('１００')
        case 2: return state.write('１０')
        case 3: return state.write('００分００秒')
        case 4: return state.write('００分００秒')
        case 6: return state.write('００分００秒')
      }
    },
    toText: (state, argument) => {
      switch (argument) {
        ...
        case 1: return '１００'
        case 2: return '１０'
        case 3: return '００分００秒'
        case 4: return '００分００秒'
        case 6: return '００分００秒'
      }
    }
  },
  0x81CB: {
...
for (let [offset, codes] of Object.entries(hylianMapping)) {
  offset = parseInt(offset)
  let tag1 = `{${codes[1]}}`
  let tag2 = `{${codes[0]}}`
  let code = 0x8440 + offset
  controlCodes[code] = {
    toBinary: (str) => {
      if (str.slice(0, tag1.length) === tag1) {
        return [tag1.length, [code >> 8, code & 0xFF]]
      } else if (str.slice(0, tag2.length) === tag2) {
        return [tag2.length, [code >> 8, code & 0xFF]]
      }
    },
    toMarkup: tag1,
    render: true,
